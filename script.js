...
function updateGPA() {
  ...
  for (let year = 1; year <= numYears; year++) {
    for (let semester = 1; semester <= 2; semester++) {
      const numClasses = parseInt(document.querySelector(`[data-year="<span class="math-inline">\{year\}"\]\[data\-semester\="</span>{semester}"] .numClasses`).value);
  ...
  resultContainer.innerHTML = `
    Unweighted GPA: <span style="color: <span class="math-inline">\{color\};"\></span>{unweightedGPA.toFixed(2)}</span><br>
    Weighted GPA: <span style="color: <span class="math-inline">\{color\};"\></span>{weightedGPA.toFixed(2)}</span>
  `;
}
